...
class _WindowsStyleTZ(tzinfo):
    ...
    Time``).  POSIX systems return short abbreviations like ``PDT``, but
    Windows returns the full descriptive name, which is what the gateway emits
    when no IANA timezone is configured (``tz`` is ``None`` and the code falls
    back to ``astimezone()``)."""
