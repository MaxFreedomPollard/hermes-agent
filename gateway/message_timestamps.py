...
# spaces (``Pacific Daylight Time``). A space-free token only matched the POSIX
# shape, so a prefix this module formatted on Windows could never be stripped
# again -- breaking the strip/render idempotence and accumulating
# ``[timestamp] [timestamp] ...`` prefixes. Match one or more space-separated
# tokens so the parser recognises every name the formatter can emit.
